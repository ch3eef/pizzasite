// Importing necessary modules
import Image from "next/image";
import Link from "next/link";
import React from "react";

// Defining the LoginPage component

const LoginPage = () => {
  return (
    // Overall container for the login page
    <div className="p-4 h-[calc(120vh-6rem)] md:h-[calc(100vh-9rem)] flex items-center justify-center">
      
      {/* BOX */}
      {/* Container for the login content, including image and form */}
      <div className=" h-full shadow-2xl rounded-md flex flex-col md:flex-row md:h-[70%] md:w-full lg:w-[60%] 2xl:w-1/2">
       
        {/* IMAGE CONTAINER */}
        {/* Container for displaying an image related to login */}
        <div className="relative h-1/3 w-full md:h-full md:w-1/2">
          {/* Using Next.js Image component to efficiently handle and display images */}
          <Image src="/loginBg.png" alt="" fill className="object-cover"/>
        </div>

        {/* FORM CONTAINER */}
        {/* Container for the login form and related elements */}
        <div className="p-10 flex flex-col gap-8 md:w-1/2">
          <h1 className="font-bold text-xl xl:text-3xl">Welcome</h1>
          <p>Log into your account or create a new one using social buttons</p>

          {/* Google Sign-In Button */}
          <button className="flex gap-4 p-4 ring-1 ring-orange-100 rounded-md">
            <Image
              src="/google.png"
              alt=""
              width={20}
              height={20}
              className="object-contain"
            />
            <span>Sign in with Google</span>
          </button>

          {/* Facebook Sign-In Button */}
          <button className="flex gap-4 p-4 ring-1 ring-blue-100 rounded-md">
            <Image
              src="/facebook.png"
              alt=""
              width={20}
              height={20}
              className="object-contain"
            />
            <span>Sign in with Facebook</span>
          </button>
          <p className="text-sm">
            Have a problem?<Link className="underline" href="/"> Contact us</Link>
          </p>
        </div>
      </div>
    </div>
  );
};

// Exporting the LoginPage component
export default LoginPage;